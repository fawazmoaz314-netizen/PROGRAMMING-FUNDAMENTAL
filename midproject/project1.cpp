...
int main()
{
    ...
    const int Total_Items = 20;  // Fixed menu items [cite: 20]
    const int History_Size = 20; // Limited bill history [cite: 89]

    // Inventory Arrays [cite: 18, 21]
    ...
    int Item_CategoryArray[Total_Items] = {1, 2, 1, 3, 1, 0}; // 1=Drink, 2=Snack, 3=Meal [cite: 27]

    // Bill History Storage [cite: 87, 90]
    ...
    // Billing Constants [cite: 54]
    float discount_threshold = 1000.0;
    float discount_rate = 0.10; // 10% discount [cite: 59]
    ...
    while (true)
    {
        ...
        if (mainOption == "1")
        {
            ...
            if (uname == "admin" && pass == "123")
            {
                while (true)
                {
                    ...
                    {
                        // --- Validation & Conflict Detection [cite: 96, 97] ---
                        system("cls");
                    ...
                    {
                        // --- Reports & Summaries [cite: 104, 105] ---
                        system("cls");
                        int rev = 0, inv = 0;
                        for (int i = 0; i < total_bills_generated; i++)
                        {
        ...
        {
            // --- Customer Billing Section [cite: 34, 129] ---
            system("cls");
            ...
            while (true)
            {
                ...
                if (fIdx != -1)
                {
                    ...
                    {
                        subtotal = subtotal + (Item_PriceArray[fIdx] * qty); // Calculate line total [cite: 45]
            ...
            // Finalize Bill and Calculate Discount [cite: 58, 60]
            if (subtotal > 0)
            {
                int discount = 0;
